...
class BondMetrics(object):
    ...
    def process(self):
        ...
        self.bonds_df = self.bonds_df.drop(self.bonds_df.columns[0], axis=1)[1:]
        self.bonds_df["timestamp"] = pd.to_datetime(self.bonds_df["timestamp"], unit="s")
        self.supply_growth_df = self.select_columns(self.bonds_df, ["timestamp", "supply"])
        ...
        self.market_cap_df = self.select_columns(
            self.bonds_df, ["timestamp", "supply", "market_price"]
        )
        ...
        self.percent_staked_df = self.select_columns(
            self.bonds_df, ["timestamp", "supply", "staked"]
        )
        ...
        self.last_df = self.bonds_df.sort_values("timestamp").tail(1)
